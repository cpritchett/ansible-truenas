...
def check_path_exists(api_client, instance_id, path):
    """Check if a path exists in the container"""
    command = f"test -e {shlex.quote(path)}"
    result = execute_command(api_client, instance_id, command, timeout=10)
    return result['rc'] == 0


def execute_command(api_client, instance_id, command, timeout=300, environment=None, chdir=None):
